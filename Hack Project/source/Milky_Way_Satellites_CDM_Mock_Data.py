...
import Milky_Way_Satellites_CDM_Model
...
# Datafile name
data_dir = '../mock_data/'
filename = 'Milky_Way_Satellites_CDM_Mock_Data'

# Set parameters
m_WDM = 20.0 # keV
sigma_0_o_m_c = 0.001 # cm^2/g, The normalization factor in the velocity dependent
                      # dark matter self-interaction cross section over mass
v_SIDM_c = 70.0 # km/s, The velocity scale above which the sigma_0_o_m_c falls off as v^-4, in units of km/s.
log_M_min = 6.0 # threshold for star formation
...
# Make dataset
M_peak, v_peak, c_peak, r_vir, x12, Halo_r12, Mr, z_infall, r_peri, r_apoc, t_infall, Torb_infall, num_total_orbits, sigma_o_m, t_over_t0, r_core, T_maxbyT_peri, f_bound, r_fact, v_fact, r_cut, Prob_surv, sigma_star, nfw, easy_core, hard_core, Prob_s_ML = Milky_Way_Satellites_CDM_Model.gen_subhalo_pop(20.0,0.0001,0.0001,M_min=10**log_M_min, alpha=alpha,beta=beta,K0=K0, alpha_m=alpha_m,sigma_m=sigma_m, mulim=mulim,Mrlim=Mrlim, M_host=M_host,sigma_r=sigma_r, A=10**-3*A,n=n,sigma_c_0=sigma_c_0, zeta=zeta,full_out=True)


M_peak_sim, v_peak_sim, c_peak_sim, r_vir_sim, x12_sim, Halo_r12_sim, Mr_sim, z_infall_sim, r_peri_sim, r_apoc_sim, t_infall_sim, Torb_infall_sim, num_total_orbits_sim, sigma_o_m_sim, t_over_t0_sim, r_core_sim, T_maxbyT_peri_sim, f_bound_sim, r_fact_sim, v_fact_sim, r_cut_sim, Prob_surv_sim, sigma_star_sim, nfw_sim, easy_core_sim, hard_core_sim, Prob_s_ML_sim  = Milky_Way_Satellites_CDM_Model.gen_subhalo_pop(m_WDM,sigma_0_o_m_c,v_SIDM_c,M_min=10**log_M_min, alpha=alpha,beta=beta,K0=K0, alpha_m=alpha_m,sigma_m=sigma_m, mulim=mulim,Mrlim=Mrlim, M_host=M_host,sigma_r=sigma_r, A=10**-3*A,n=n,sigma_c_0=sigma_c_0, zeta=zeta, full_out=True)
...
fig2.savefig('../plots/Velocity_Dispersion_vs_Luminosity_Milky_Way_Satellites_CDM_Mock_Data.pdf',bbox_inches='tight')
