...
def gen_subhalo_pop(m_WDM, sigma_0_o_m_c = 150.0, v_SIDM_c = 50.0, M_min=10**7.5, mulim=30, Mrlim=-1, alpha=1.87, beta=1.16,
    ...
    # TODO: Derive CDF
    mvec = np.linspace(np.log10(M_min),np.log10(M_max),100)
    ...
    if full_out:
        return 10**log_M_peak[idx_s], vpeak, c_peak[idx_s], rvir[idx_s], x12, 1000*Halo_r12, Mr[idx_s], zinfall, r_peri, r_apoc, t_infall, Torbinfall, num_total_orbits, sigma_o_m, t_over_t0, rcore, 10**log10_T_maxbyT_peri, 10**log10_f_bound, rfact, vfact, r_cut, Prob_s, sigmastar, nfw, easy_core, hard_core, Prob_s_1[idx_s]
    else:
        return  Prob_s, sigmastar, Mr[idx_s]
...
def like_sat(in_vec, dat, MV_edge=np.array([-16,-12,-8,-6,-4,-2,0]), sig_edge=np.array([0,2,4,6,8,10,20]),                             mulim=30., Mrlim=-1., alpha=1.87, beta=1.16, K0=1.88e-3):
    ...
    if m_WDM < 1.0 or m_WDM > 20.0:
        return -np.inf
    if sigma_0_o_m_c < 1.0 or sigma_0_o_m_c > 500.0:
        return -np.inf
    ...
    # Add m_WDM prior
    like *= (1.0/m_WDM)*(1/np.log(20.0/1.0))
